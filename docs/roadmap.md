...
| Operations and release | partial | `doctor --gateway`, foreground and daemon start/status/graceful-stop control, storage status, retention GC, targeted purge, backup, verification, restore, resumable key rotation, and packaged resources | metrics, load/soak/chaos suites, canary rollout, and rehearsed live recovery |
