...
| Multi-Agent supervisor | partial | durable rooted graph, bounded delegation, child Harnesses, typed artifacts, same-requester root cancellation cascade, read-only parallel children, live-resolved existing-resource writer locks, managed-title logical create reservations, exact child write scopes, root-only completion, and three-worker restart-chaos acceptance | production child mailbox commands and many-graph fairness evidence |
