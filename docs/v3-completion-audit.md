...
| --- | --- | --- | --- |
| Admission enablement and actor authorization | Requirements 4.1 and acceptance 1/12 | `IMAdmissionService` checks group/bot/mention/body, while new chats are automatically stored as enabled and no actor policy is consulted | Trusted admission policy rejects disabled groups and unauthorized actors before mirroring, task creation, acknowledgement, rollout selection, or model work |
