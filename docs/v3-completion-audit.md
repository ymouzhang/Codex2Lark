...
| Wall-time and production cost budgets — wall-time resolved | Requirements 4.3 and acceptance 5 | The Harness now persists cumulative active monotonic time and bounds model/tool awaits by the remaining allowance; production cost accounting and declared production limits remain open | Direct timeout/resume tests pass; production cost calculation and configured wall/cost limits are still required |
